<a href="{{route('admin.recruiter.show', $model)}}" class="btn btn-info btn-sm m-2" title="Информация">
    <i class="fas fa-file-alt"></i>
</a>
@if($model->status === \App\Models\Auth\User::STATUS_MODERATE)
    <button type="button" class="btn btn-success btn-sm m-2" wire:click="activate({{$model->id}})" title="Подтвердить">
        <i class="fas fa-clipboard-check"></i></button>
@endif

@if($model->status === \App\Models\Auth\User::STATUS_HIDDEN)
    <button type="button" class="btn btn-success btn-sm m-2" wire:click="activate({{$model->id}})" title="Активировать">
...
@if($model->status === \App\Models\Auth\User::STATUS_ACTIVE || $model->status === \App\Models\Auth\User::STATUS_HIDDEN || $model->status === \App\Models\Auth\User::STATUS_MODERATE)
    <button type="button" class="btn btn-danger btn-sm m-2" wire:click="lock({{$model->id}})" title="Заблокировать"><i
