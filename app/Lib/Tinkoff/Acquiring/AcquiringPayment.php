...
namespace App\Lib\Tinkoff\Acquiring;
...
class AcquiringPayment
{
    ...
    {
        $this->acquiring_url = config('services.tinkoff.sandbox')
            ? 'https://rest-api-test.tinkoff.ru/v2/'
            : 'https://securepay.tinkoff.ru/v2/';

        $this->terminal_id = config('services.tinkoff.terminal');
        $this->secret_key = config('services.tinkoff.password');
        $this->setupUrls();
    }
    ...
    public function paymentURL(array $payment, array $items)
    {
        ...
        foreach ($items as $item) {
            ...
            $payment['Items'][] = [
                ...
                'Amount' => $item['Amount'] * $amount_multiplicator,
                'Tax' => $item['Tax'],
            ];
        }
    ...
    {
        $keys = ['Name', 'Price', 'Amount', 'Tax', 'Quantity'];
        return $this->allKeysIsExistInArray($keys, $array_for_check);
    }
